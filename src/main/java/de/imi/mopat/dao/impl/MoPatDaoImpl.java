...
public abstract class MoPatDaoImpl<T> implements MoPatDao<T> {
    ...
    private static final org.slf4j.Logger LOGGER = org.slf4j.LoggerFactory.getLogger(MoPatDaoImpl.class);
    ...
    public void merge(final T element) {
        try {
            ...
            LOGGER.error(
                    "Error while saving entity of type {}. Check if there is " + "a consistent database.",
    ...
    public void grantRight(final T element, final User user, final PermissionType right, final Boolean inheritance) {
        try {
            ...
            AclClass elementClass =
                    aclClassDao.getElementByClass(getEntityClass().getName());
            // Cast the element to the appropriate entity class
            getEntityClass().cast(element);
            // Get the getId method for the element
            ...
            AclObjectIdentity elementObjectIdentity =
                    aclObjectIdentityDao.getElementByClassAndObjectId(elementClass, elementId);
            AclEntry elementUserAccess = aclEntryDao.getEntryForObjectUserAndRight(element, user, right);
            // If the user does not have the given right for the given object
            // . grant it
            if (elementUserAccess == null) {
                elementUserAccess = new AclEntry(user, elementObjectIdentity, 1, right, true, false, false);
            ...
            if (inheritance) {
                ...
                | SecurityException ex) {
            LOGGER.error(
                    "Error during granting the right{} for entity of type {} "
                            + "and user with ID {}. Check if there is a " + "consistent database.",
                    right,
                    getEntityClass().getName(),
                    user.getId());
        }
