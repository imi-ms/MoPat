...
public class ConfigurationGroupDaoImpl extends MoPatDaoImpl<ConfigurationGroup> implements ConfigurationGroupDao {
    ...
    public List<ConfigurationGroup> getConfigurationGroups(final String labelMessageCode) {
        try {
            Query query = moPatEntityManager.createQuery(
                    "SELECT c FROM " + "ConfigurationGroup c where c.labelMessageCode = :labelMessageCode");
    ...
    public ConfigurationGroup getConfigurationGroupByName(final String name) {
        try {
            Query query =
                    moPatEntityManager.createQuery("SELECT c FROM " + "ConfigurationGroup c where c.name = :name");
    ...
    public boolean isConfigurationGroupDeletable(final Long configurationGroupId) {
        ...
        TypedQuery<Long> existingClinicConfigurationMappingsQuery = moPatEntityManager.createQuery(
                "SELECT count (ccgm) FROM ClinicConfigurationGroupMapping ccgm WHERE ccgm" + ".configurationGroup.id = "
