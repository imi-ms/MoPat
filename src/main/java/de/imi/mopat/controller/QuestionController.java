...
public class QuestionController {
    ...
    private final QuestionType[] equalQuestionTypesSlider = {QuestionType.SLIDER, QuestionType.NUMBER_CHECKBOX};
    private final QuestionType[] equalQuestionTypesSelect = {QuestionType.MULTIPLE_CHOICE, QuestionType.DROP_DOWN};
    private final List<QuestionType> equalQuestiontypesSliderList = Arrays.asList(equalQuestionTypesSlider);
    private final List<QuestionType> equalQuestiontypesSelectList = Arrays.asList(equalQuestionTypesSelect);
    ...
    public String showQuestions(@RequestParam(value = "id", required = true) final Long id, final Model model) {
        ...
        for (Question question : questionnaire.getQuestions()) {
            ...
            // question
            SortedMap<String, Map<String, String>> groupedLocalizedQuestionTextByCountry =
            ...
            localizedQuestionTextsForQuestion.put(question.getId(), groupedLocalizedQuestionTextByCountry);
    ...
    public String editQuestion(
            ...
            @RequestParam(value = "id", required = false) final Long questionId,
            final Model model) {
        ...
        if (questionId != null && questionId > 0) {
            ...
            if (question != null) {
                ...
                if (questionDTO.getQuestionType() == QuestionType.DROP_DOWN
                    ...
                    for (Map.Entry<Long, AnswerDTO> answerEntry :
                            questionDTO.getAnswers().entrySet()) {
                        answerEntry
                                .getValue()
                                .setHasConditionsAsTarget(conditionDao.isConditionTarget(answerDao.getElementById(
                                        answerEntry.getValue().getId())));
                    }
                }
            }
        }
    ...
    public String duplicateQuestion(
            @RequestParam(value = "questionnaireId", required = false) final Long questionnaireId, final Model model) {
        ...
        if (oldQuestion.getId() != null) { // [bt] getting the old Question
            ...
            BeanPropertyBindingResult errors = new BeanPropertyBindingResult(newQuestion, "question");
            errors.reject(
                    "errormessage",
                    messageSource.getMessage(
                            "question.error" + ".duplicateQuestionMissing",
                            new Object[] {},
        ...
        model.addAttribute("questionDTO", questionDTOMapper.apply(newQuestion));
        model.addAttribute("questionnaireId", questionnaireId);
    ...
    public String editQuestion(
        ...
        Questionnaire questionnaire = questionnaireDao.getElementById(questionDTO.getQuestionnaireId());
        ...
        if (questionDTO.getId() != null) {
            updatePropertiesOfExistingQuestion(
                    questionDTO, question, localizedQuestionText, minNumberAnswers, maxNumberAnswers);
        } else {
            question = createQuestion(
                    questionDTO, localizedQuestionText, questionnaire, minNumberAnswers, maxNumberAnswers);
        ...
        questionDTO
                .getQuestionType()
                .getStrategy()
                .createOrUpdateAnswer(questionDTO, question, this, result, questionnaire);
        ...
        if (questionDTO.getQuestionType().equals(QuestionType.IMAGE)
            ...
            try {
                ...
                questionDTO
                        .getAnswers()
                        .get(0L)
                        .setImageBase64(StringUtilities.convertImageToBase64String(realPath, fileName));
            } catch (IOException e) {
        ...
        if (answerDTO.getBodyPartMessageCode() != null) {
            ...
            for (String locale : questionDTO.getLocalizedQuestionText().keySet()) {
                localizedLabel.put(
                        locale,
                        messageSource.getMessage(
                                answerDTO.getBodyPartMessageCode(),
                                new Object[] {},
    ...
    private void deleteScoresIfNecessary(QuestionDTO questionDTO, Question question) {
        ...
        if (scoreDao.hasScore(question)) {
            List<QuestionType> validScoreQuestionTypes = new ArrayList<>(Arrays.asList(
                    QuestionType.MULTIPLE_CHOICE,
                    QuestionType.SLIDER,
                    QuestionType.NUMBER_CHECKBOX,
                    QuestionType.NUMBER_CHECKBOX_TEXT,
            ...
            if (!validScoreQuestionTypes.contains(questionDTO.getQuestionType())
                    || questionDTO.getQuestionType().equals(QuestionType.MULTIPLE_CHOICE)
                            && (questionDTO.getMinNumberAnswers() != 1 || questionDTO.getMaxNumberAnswers() != 1)) {
    ...
    private void deleteImageIfNecessary(QuestionDTO questionDTO, Question question) {
        if (question.getQuestionType().equals(QuestionType.IMAGE)) {
            ...
            if (!(questionDTO.getQuestionType().equals(QuestionType.IMAGE)
                ...
                File deleteFile = new File(((ImageAnswer) question.getAnswers().get(0)).getImagePath());
                deleteFile.delete();
    ...
    private void setMinMaxNumberOfAnswers(
            QuestionDTO questionDTO,
            List<QuestionType> equalQuestiontypesSelectList,
            Question question,
            Integer minNumberAnswers,
            Integer maxNumberAnswers) {
    ...
    private void updatePropertiesOfExistingQuestion(
            QuestionDTO questionDTO,
            Question question,
        ...
        setMinMaxNumberOfAnswers(
                questionDTO, equalQuestiontypesSelectList, question, minNumberAnswers, maxNumberAnswers);
    }
    ...
    private Question createQuestion(
        ...
        question = new Question(
                ...
                questionDTO.getIsEnabled(),
                questionDTO.getQuestionType(),
                (questionnaire.getQuestions().size() + 1),
                questionnaire,
                questionDTO.getIsJustInfo());
        // If dropdown question set min/max number of answers to 1
        if (question.getQuestionType() == QuestionType.DROP_DOWN) {
            minNumberAnswers = 1;
            maxNumberAnswers = 1;
        } else if (questionDTO.getMaxNumberAnswers() == null || questionDTO.getMaxNumberAnswers() == 0) {
            maxNumberAnswers = 1;
        }
        question.setMinMaxNumberAnswers(minNumberAnswers, maxNumberAnswers);
        question.setCodedValueType(questionDTO.getCodedValueType());
    ...
    @PreAuthorize("hasRole('ROLE_EDITOR')")
    public String removeQuestion(@RequestParam(value = "id", required = true) final Long id, final Model model) {
        ...
        if (!questionToDelete.isDeletable()) {
            errorMessages.add(messageSource.getMessage(
                    "question.label.deleteQuestionNotPossible", new Object[] {}, LocaleContextHolder.getLocale()));
        ...
        if (questionToDelete.getQuestionType().equals(QuestionType.IMAGE)) {
            File deleteFile =
                    new File(((ImageAnswer) questionToDelete.getAnswers().get(0)).getImagePath());
            deleteFile.delete();
        }
            ...
            for (Answer answer : question.getAnswers()) {
                for (Condition condition : answer.getConditions()) {
                    for (int j = 0; j < i; j++) {
                        ...
                        // of this condition
                        for (Answer otherAnswer :
                                questionDao.getElementById(questionIds.get(j)).getAnswers()) {
                            if (Objects.equals(condition.getTarget().getId(), otherAnswer.getId())) {
                                return messageSource.getMessage(
                                        "questionnaire.questions.reposition" + ".conditionError",
                                        new Object[] {},
                                        LocaleContextHolder.getLocale());
