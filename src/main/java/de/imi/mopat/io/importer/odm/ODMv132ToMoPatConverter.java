...
public class ODMv132ToMoPatConverter {
     ...
     */
    public static final ImportQuestionnaireResult convertToQuestionnaire(
            MultipartFile importedODM,
        ...
        if (name.length() < questionnaireNameMinLength) {
            LOGGER.debug(
                    "The name ('{}') of FormDef of OID {} was shorter "
                            + "than {} characters (minimum questionnaire name length in "
                            + "MoPat), so I'll take its OID instead.",
                    name,
                    formDef.getOID(),
                    questionnaireNameMinLength);
            result.addValidationMessage("import.odm.v132.formDef.nameTooShort", new String[] {formDef.getOID(), name});
            name = formDef.getOID().trim();
            if (name.length() < questionnaireNameMinLength) {
                LOGGER.debug(
                        "The OID ('{}') of the FormDef was too short as well,"
                                + " so I'll put some underscores in front of it.",
                        name);
                result.addValidationMessage("import.odm.v132.formDef.oidTooShort", new String[] {formDef.getOID()});
                do {
                    name = "_".concat(name);
        ...
        if (name.length() > questionnaireNameMaxLength) {
            LOGGER.debug(
                    "The name of FormDef of OID {} was larger than {} "
                            + "characters (maximum questionnaire name length in MoPat), so "
                            + "I'll cut it after {} characters.",
                    formDef.getOID(),
                    questionnaireNameMaxLength,
                    questionnaireNameMaxLength);
            result.addValidationMessage("import.odm.v132.formDef.nameTooLong", new String[] {formDef.getOID(), name});
        ...
        if (odmDescription == null) {
            LOGGER.debug(
                    "The FormDef of OID {} did not provide a Description. " + "Will take the name ('{}') instead.",
                    formDef.getOID(),
                    name);
            result.addValidationMessage(
                    "import.odm.v132.formDef.descriptionNull", new String[] {formDef.getOID(), name});
            description = name;
        } else {
            LOGGER.debug(
                    "The FormDef of OID {} has a Description. Will iterate "
                            + "over its TranslatedTexts to find a proper one.",
                    formDef.getOID());
            Map<String, String> translatedLocalizedText =
                    getBestFittingTranslatedText(odmDescription.getTranslatedText(), questionnaireNameMinLength);
            if (translatedLocalizedText == null) {
                LOGGER.debug(
                        "The FormDef of OID {} did not have a "
                ...
                LOGGER.debug(
                        "The FormDef of OID {} has a "
                                + "Description->TranslatedText of minimum length ({}) and the"
                                + " matching lang/default attribute. Questionnaire "
                                + "description will be set to: {}",
                        formDef.getOID(),
                        questionnaireNameMinLength,
                        translatedLocalizedText);
                description =
                        translatedLocalizedText.entrySet().iterator().next().getValue();
            }
        }

        boolean isPublished = false;
        LOGGER.debug(
                "All info for Questionnaire complete (name: '{}', "
                        + "description: '{}', changedBy: '{}', isPublished: '{}'). "
                        + "Will now create and set it.",
                name,
                description,
                changedBy,
                isPublished);
        ...
        if (itemGroupRefList == null) {
            LOGGER.debug(
                    "The FormDef of OID {} did not contain any ItemGroupRefs." + " Won't create any question(group)s",
                    formDef.getOID());
            result.addValidationMessage("import.odm.v132.formDef.itemGroupRefNull", new String[] {formDef.getOID()});
        } else {
            LOGGER.debug(
                    "The FormDef of OID {} contains at least one ItemGroupRef"
            ...
            if (itemGroupDefList == null || itemGroupDefList.isEmpty()) {
                ...
                LOGGER.debug(
                        "MetaDataVersion of OID {} contains at least one"
                ...
                for (ODMcomplexTypeDefinitionItemGroupRef itemGroupRef : itemGroupRefList) {
                    ...
                    for (ODMcomplexTypeDefinitionItemGroupDef itemGroupDef : itemGroupDefList) {
                        LOGGER.debug(
                                "Now checking the ItemGroupDef with OID {} "
                ...
                Map<Integer, ODMcomplexTypeDefinitionItemGroupDef> orderedItemGroupDefs =
                        new HashMap<Integer, ODMcomplexTypeDefinitionItemGroupDef>();
                for (ODMcomplexTypeDefinitionItemGroupRef itemGroupRef : matchedItemGroupRefs) {
                    ...
                    if (orderNumber == null) {
                        LOGGER.debug(
                                "The ItemGroupRef with OID {} does not have "
                        ...
                        if (orderedItemGroupDefs.get(orderNumber.intValue()) == null) {
                            LOGGER.debug(
                                    "The List of ItemGroupDefs has a "
                            ...
                            LOGGER.debug(
                                    ...
                                    orderNumber.intValue());
                            result.addValidationMessage(
                                    "import.odm.v132.formDef.itemGroupRef" + ".multipleOrderNumber", new String[] {
                                        formDef.getOID(), itemGroupRef.getItemGroupOID(), orderNumber.toString()
                                    });
                            orderNumberForAllItemGroupRefsPresent = false;
                        }
                    }
                ...
                if (orderNumberForAllItemGroupRefsPresent) {
                    LOGGER.debug(
                            ...
                            metaDataVersion.getOID(),
                            formDef.getOID());
                    List<Integer> orderNumbers = new ArrayList<Integer>(orderedItemGroupDefs.keySet());
                    ...
                    LOGGER.debug(
                            "Not all ItemGroupDefs of MetaDataVersion of"
                ...
                LOGGER.debug(
                        "Collection of all necessary info for converting "
                                + "refered ItemGroupDefs of FormDef of OID {} done. "
                                + "Will now iterate over them and do the conversion.",
                        formDef.getOID());
                for (ODMcomplexTypeDefinitionItemGroupDef itemGroupDef : itemGroupDefListToIterateOver) {
                    LOGGER.debug("Calling conversion method for ItemGroupDef of " + "OID {}.", itemGroupDef.getOID());
                    int currentHighestQuestionPosition = result.getNumberOfQuestions();
                    result.addImportQuestionListResult(convertToQuestionList(
                            itemGroupDef,
                            questionnaire,
                            currentHighestQuestionPosition,
                            metaDataVersion,
                            exportTemplates,
                            messageSource));
                }
                    ...
                    for (ImportConditionResult condition : odmQuestion.getConditions()) {
                        if (condition.getTargetIdentifier() != null
                            ...
                            for (ImportQuestionListResult questionListCondition : result.getQuestionListResults()) {
                                if (questionListCondition.getIdentifier().equals(condition.getTargetIdentifier())) {
                                    // Look for the ItemData containing the
                                    // trigger ItemDef OID
                                    boolean questionTriggerFound = false;
                                    for (ImportQuestionResult odmQuestionCondition :
                                            questionList.getImportQuestionResults()) {
                                        if (odmQuestionCondition
                                                .getIdentifier()
                                            ...
                                            if (odmQuestionCondition
                                                ...
                                                for (int i = 0;
                                                        i
                                                                < odmQuestionCondition
                                                                        .getQuestion()
                                                                        .getAnswers()
                                                                        .size();
                                                        i++) {
                                                    Double scoreValue = ((SelectAnswer) odmQuestionCondition
                                                                    .getQuestion()
                                                                    .getAnswers()
                                                                    .get(i))
                                                            .getValue();
                                                    if (scoreValue != null
                                                            && scoreValue.equals(
                                                                    Double.parseDouble(condition.getTriggerValue()))) {
                                                        // Add a new condition
                                                        ...
                                                        odmQuestion.addValidationMessage(
                                                                "import.odm" + ".v132" + ".conditionDef.ScoreMissing",
                                                                new String[] {
                                                                    odmQuestion.getIdentifier(),
                                                                    odmQuestionCondition.getIdentifier()
                                                                });
                                    ...
                                    if (!questionTriggerFound) {
                                        odmQuestion.addValidationMessage(
                                                "import.odm.v132.conditionDef" + ".TriggerQuestionNotFound",
                            ...
                            if (condition.getTargetIdentifier() == null) {
                                odmQuestion.addValidationMessage(
                                        "import.odm.v132.conditionDef" + ".ConditionMissingItemGroupData",
                                        new String[] {odmQuestion.getIdentifier()});
                            }
    ...
    public static final ImportQuestionListResult convertToQuestionList(
            ...
            int currentHighestQuestionPosition,
            ODMcomplexTypeDefinitionMetaDataVersion metaDataVersion,
            List<ExportTemplate> exportTemplates,
            MessageSource messageSource) {
        LOGGER.debug("Enter convertToQuestionList" + "(ODMcomplexTypeDefinitionItemGroupDef, Questionnaire, "
                + "ODMcomplexTypeDefinitionMetaDataVersion)");
        ...
        LOGGER.debug("Checking the list of ItemRefs for ItemGroupDef of OID {}", itemGroupDef.getOID());
        List<ODMcomplexTypeDefinitionItemRef> itemRefList = itemGroupDef.getItemRef();
        if (itemRefList == null || itemRefList.isEmpty()) {
            LOGGER.debug(
                    "The ItemGroupDef of OID {} did not provide a List of "
                            + "ItemRefs. Since MoPat needs them (they are the "
                            + "questions), this ItemGroupDef will not be converted. "
                            + "Creating an error message and leaving.",
                    itemGroupDef.getOID());
            result.addValidationMessage(
                    "import.odm.v132.itemGroupDef.itemRefListNullEmpty", new String[] {itemGroupDef.getOID()});
        } else {
            LOGGER.debug(
                    "The ItemGroupDef of OID {} does provide a List of " + "ItemRefs. Will now check for ItemDefs in "
                            + "MetaDataVersion of OID {}",
                    itemGroupDef.getOID(),
                    metaDataVersion.getOID());
            List<ODMcomplexTypeDefinitionItemDef> itemDefList = metaDataVersion.getItemDef();
            if (itemDefList == null || itemDefList.isEmpty()) {
                LOGGER.debug(
                        "The MetaDataVersion of OID {} did not contain "
                        ...
                        itemGroupDef.getOID());
                result.addValidationMessage(
                        "import.odm.v132.metaDataVersion.itemDefListNullEmpty",
                        new String[] {metaDataVersion.getOID()});
            } else {
                LOGGER.debug(
                        "The MetaDataVersion of OID {} contains at least"
                                + " one ItemDef. Will now iterate over them and the ItemRefs "
                                + "in ItemGroupDef of OID {} to collect the ItemDefs.",
                        metaDataVersion.getOID(),
                        itemGroupDef.getOID());
                Map<ODMcomplexTypeDefinitionItemRef, ODMcomplexTypeDefinitionItemDef> matchingItemRefDefs =
                        new HashMap<ODMcomplexTypeDefinitionItemRef, ODMcomplexTypeDefinitionItemDef>();
                List<ODMcomplexTypeDefinitionItemRef> matchedItemRefs =
                        new ArrayList<ODMcomplexTypeDefinitionItemRef>();
                List<ODMcomplexTypeDefinitionItemDef> matchedItemDefs =
                        new ArrayList<ODMcomplexTypeDefinitionItemDef>();
                Map<ODMcomplexTypeDefinitionItemDef, Boolean> itemDefIsRequiredMap = new HashMap<>();
                Map<ODMcomplexTypeDefinitionItemDef, ArrayList<ODMcomplexTypeDefinitionFormalExpression>>
                        itemDefConditionFormalExpression = new HashMap<>();
                for (ODMcomplexTypeDefinitionItemRef itemRef : itemRefList) {
                    LOGGER.debug(
                            "Now checking for an ItemDef with OID {} (because"
                    ...
                    for (ODMcomplexTypeDefinitionItemDef itemDef : itemDefList) {
                        LOGGER.debug(
                                "Now checking the ItemDef with OID {} for " + "equality with ItemRef of ItemOID {}",
                                itemDef.getOID(),
                                itemRef.getItemOID());
                        if (refItemOID.equalsIgnoreCase(itemDef.getOID())) {
                            LOGGER.debug(
                                    "ItemDef with OID {} matched the reffered"
                                            + " OID. Will put it into the list of "
                                            + "ItemDefs to make Question out of",
                                    itemDef.getOID());
                            ...
                            LOGGER.debug(
                                    "While putting the ItemDef of OID {}"
                            ...
                            ArrayList<ODMcomplexTypeDefinitionFormalExpression> formalExpressionsItemDef =
                                    new ArrayList<>();
                            ...
                            for (ODMcomplexTypeDefinitionConditionDef condition : metaDataVersion.getConditionDef()) {
                                if (condition.getOID().equals(collectionExceptionConditionOID)) {
                                    formalExpressionsItemDef = (ArrayList<ODMcomplexTypeDefinitionFormalExpression>)
                                            condition.getFormalExpression();
                                }
                            }
                            LOGGER.debug(
                                    "While putting the ItemDef of OID {}"
                    ...
                    if (!itemRefFoundInItemDef) {
                        LOGGER.debug(
                                "Iteration over ItemDefs in "
                                        ...
                                        + "will not be converted/considered.",
                                metaDataVersion.getOID(),
                                itemRef.getItemOID());
                        result.addValidationMessage(
                ...
                LOGGER.debug(
                        "Iteration over ItemRefs in ItemGroupDef of OID "
                ...
                for (ODMcomplexTypeDefinitionItemRef itemRef : matchedItemRefs) {
                    LOGGER.debug("Now checking for an OrderNumber for ItemRef of " + "OID {}", itemRef.getItemOID());
                    BigInteger orderNumber = itemRef.getOrderNumber();
                    if (orderNumber == null) {
                        LOGGER.debug(
                                "The ItemRef with OID {} does not have an "
                        ...
                        LOGGER.debug(
                                "The ItemRef with OID {} has an "
                                        + "OrderNumber, so I'll try to put the referenced "
                                        + "ItemDef onto its position ({}) in the List",
                                itemRef.getItemOID(),
                                orderNumber.intValue());
                        if (orderedItemDefs.get(orderNumber.intValue()) == null) {
                            LOGGER.debug(
                                    "The List of ItemDefs has a free spot at "
                                            + "position/OrderNumber {}, so I'll just "
                                            + "put the ItemDef of OID {} there.",
                                    orderNumber.intValue(),
                                    itemRef.getItemOID());
                            orderedItemDefs.put(orderNumber.intValue(), matchingItemRefDefs.get(itemRef));
                        } else {
                            LOGGER.debug(
                                    "The List of ItemDefs already has an "
                            ...
                            result.addValidationMessage(
                                    ...
                                    new String[] {itemGroupDef.getOID(), itemRef.getItemOID(), orderNumber.toString()});
                            orderNumberForAllItemRefsPresent = false;
                        }
                    }
                }
                ...
                if (orderNumberForAllItemRefsPresent) {
                    LOGGER.debug(
                            "All ItemDefs of MetaDataVersion of OID {} "
                    ...
                    LOGGER.debug(
                            "Not all ItemDefs of MetaDataVersion of OID "
                ...
                for (ODMcomplexTypeDefinitionItemDef itemDef : itemDefListToIterateOver) {
                    LOGGER.debug("Calling conversion method for ItemDef of OID {}.", itemDef.getOID());
                    // [bt] set the position of the question, because we
                    // start counting with 1 in MoPat
                    int position = (currentHighestQuestionPosition
                            + result.getQuestionList().size()
                            + 1);
                    // retrieve the CollectionExceptionConditionOID
                    result.addImportQuestionResult(convertToQuestion(
                            itemDef,
                            itemDefIsRequiredMap.get(itemDef),
                            itemDefConditionFormalExpression.get(itemDef),
                            position,
                            questionnaire,
                            metaDataVersion,
                            exportTemplates,
    ...
    public static final ImportQuestionResult convertToQuestion(
            ...
            Boolean isRequired,
            ArrayList<ODMcomplexTypeDefinitionFormalExpression> formalExpressions,
            Integer position,
            Questionnaire questionnaire,
            ODMcomplexTypeDefinitionMetaDataVersion metaDataVersion,
            List<ExportTemplate> exportTemplates,
        ...
        if (odmQuestion == null) { // [bt] as defined in , an ItemDef without a
            ...
            LOGGER.debug(
                    "The Question Element of ItemDef with OID {} was null; "
                            + "but we need one for the question text. Will write an "
                            + "error message and finish.",
                    itemDef.getOID());
            result.addValidationMessage("import.odm.v132.itemDef.questionNull", new String[] {itemDef.getOID()});
        } else {
            LOGGER.debug(
                    "The Question Element of ItemDef with OID {} was not "
            ...
            Map<String, String> localizedQuestionTexts = getTranslations(translatedTextList, minQuestionTextLength);
            ...
            if (localizedQuestionTexts == null || localizedQuestionTexts.isEmpty()) {
                ...
                result.addValidationMessage(
                        "import.odm.v132.itemDef.noProperQuestion",
                        new String[] {itemDef.getOID(), Integer.toString(minQuestionTextLength)});
            } else {
                LOGGER.debug(
                        "Proper question text found ({}), now checking for "
                                + "CodedListRef --> Multiple Choice question",
                        localizedQuestionTexts);
                ODMcomplexTypeDefinitionCodeListRef codeListRef = itemDef.getCodeListRef();
                if (codeListRef == null) {
                    LOGGER.debug(
                            "The ItemDef with OID {} did not refer to a "
                    ...
                    switch (dataType) {
                        ...
                        case STRING: {
                            LOGGER.debug(
                                    "DataType {} in ItemDef of OID {} " + "detected. Will convert it into a free "
                                            + "text question.",
                                    dataType,
                                    itemDef.getOID());
                        ...
                        case INTEGER: {
                            LOGGER.debug(
                                    "DataType {} in ItemDef of OID {} "
                                            + "detected. Will check for RangeChecks "
                                            + "to decide whether to convert it into a"
                                            + " number checkbox or slider question.",
                                    dataType,
                                    itemDef.getOID());
                            Double[] minMaxFromRangeCheck =
                                    getMinMaxFromRangeCheck(itemDef, itemDef.getRangeCheck(), result);
                            Double min = minMaxFromRangeCheck[0];
                            Double max = minMaxFromRangeCheck[1];
                            Boolean vertical = DEFAULT_VERTICAL;
                            LOGGER.debug("Finished with min/max value stuff. " + "Will set the stepsize.");
                            Double stepsize = DEFAULT_INTEGER_STEPSIZE;
                            result.addValidationMessage(
                                    "import.odm.v132.itemDef.stepSize",
                                    new String[] {itemDef.getOID(), Double.toString(stepsize)});

                            if (min == null || max == null) {
                                questionType = QuestionType.NUMBER_INPUT;
                                LOGGER.debug("Minimum or maximum is null. Set" + " the question type to NUMBER INPUT.");
                                result.addValidationMessage(
                                        "import.odm.v132.itemDef.numberInput", new String[] {itemDef.getOID()});
                                ...
                                result.addValidationMessage(
                                        "import.odm.v132.itemDef" + ".numberCheckbox", new String[] {
                                            itemDef.getOID(),
                                            String.valueOf(max - min + 1),
                                ...
                                result.addValidationMessage("import.odm.v132.itemDef.slider", new String[] {
                                    itemDef.getOID(),
                                    String.valueOf(max - min + 1),
                                    String.valueOf(DEFAULT_INTEGER_MIN_MAX_DIFFERENCE)
                                });
                            ...
                            if (questionType == QuestionType.NUMBER_INPUT) {
                                ...
                                NumberInputAnswer numberInputAnswer =
                                        new NumberInputAnswer(question, true, min, max, stepsize);
                                for (ExportTemplate exportTemplate : exportTemplates) {
                                ...
                                SliderAnswer sliderAnswer =
                                        new SliderAnswer(question, true, min, max, stepsize, vertical);
                                for (ExportTemplate exportTemplate : exportTemplates) {
                        ...
                        case DOUBLE: {
                            ...
                            if (itemDef.getSignificantDigits() != null
                                ...
                                LOGGER.debug("Get the stepsize out of the " + "significant digit attribute");
                                ...
                                if (stepsize < DEFAULT_DOUBLE_STEPSIZE) {
                                    ...
                                    result.addValidationMessage(
                                            "import.odm.v132.itemDef" + ".significantDigitTooSmall", new String[] {
                                                itemDef.getOID(),
                                    ...
                                    LOGGER.debug(
                                            "If the significant digit " + "value is a supported stepsize, use it.");
                                    result.addValidationMessage(
                                ...
                                stepsize = DEFAULT_DOUBLE_STEPSIZE;
                                LOGGER.debug(
                                        "If the significant digit is not" + " set, use the default double stepsize.");
                                result.addValidationMessage(
                                        "import.odm.v132.itemDef" + ".significantDigitTooSmall", new String[] {
                                            itemDef.getOID(),
                        ...
                        case DATE: {
                            ...
                            if (rangeCheckList == null || rangeCheckList.isEmpty()) {
                                LOGGER.debug(
                                        "ItemDef of OID {} did not contain " + "any RangeChecks. Won't set any "
                                                + "start or end dates.",
                                        itemDef.getOID());
                                result.addValidationMessage(
                                ...
                                for (ODMcomplexTypeDefinitionRangeCheck rangeCheck : rangeCheckList) {
                                    LOGGER.debug(
                                            "Now checking RangeCheck at " + "position {}",
                                                ...
                                                if (startDate == null) {
                                                    LOGGER.debug(
                                                            "RangeCheck " + "comparator {} "
                                                                    ...
                                                                    + "'CheckValue'",
                                                            comparator);
                                                    List<ODMcomplexTypeDefinitionCheckValue> checkValueList =
                                                            rangeCheck.getCheckValue();
                                                    if (checkValueList == null || checkValueList.isEmpty()) {
                                                        ...
                                                        LOGGER.debug(
                                                                "RangeCheck " + "at " + "position "
                                                                        ...
                                                                        + ". Iterating over it.",
                                                                rangeCheckList.indexOf(rangeCheck));
                                                        for (ODMcomplexTypeDefinitionCheckValue checkValue :
                                                                checkValueList) {
                                                            LOGGER.debug(
                                                            ...
                                                            try {
                                                                LOGGER.debug(
                                                                        "Trying to parse the xml value ('{}') into a"
                                                                                + " date",
                                                                        checkValue.getValue());
                                                                startDate = new SimpleDateFormat(DATE_FORMAT)
                                                                        .parse(checkValue.getValue());
                                                                LOGGER.debug(
                                                                ...
                                                                LOGGER.debug(
                                                                        "ParseException when trying to parse the"
                                                                                + " CheckValue at position {} of"
                                                                                + " RangeCheck at Position: {}",
                                                                        checkValueList.indexOf(checkValue),
                                                                        rangeCheckList.indexOf(rangeCheck),
                                                                        pe);
                                                            ...
                                                            try {
                                                                LOGGER.debug(
                                                                        "Trying to parse the xml value ('{}') into a"
                                                                                + " date",
                                                                        checkValue.getValue());
                                                                endDate = new SimpleDateFormat(DATE_FORMAT)
                                                                        .parse(checkValue.getValue());
                                                                LOGGER.debug(
                                                                        "Parsing successful. End date is: {}", endDate);
                                                                break;
                                                            } catch (ParseException pe) {
                                                                LOGGER.debug(
                                                                        "ParseException when trying to parse the"
                                ...
                                if (startDate != null && endDate != null && endDate.before(startDate)) {
                                    LOGGER.debug(
                                            "Start and end date were found, "
                                                    + "but end date ({}) was not "
                                                    + "after start date ({}). Won't " + "set them.",
                                            endDate,
                                            startDate);
                                    result.addValidationMessage(
                                            "import.odm.v132.itemDef" + ".rangeCheckEndDateBeforeStartDate",
                                            new String[] {
                                                itemDef.getOID(),
                            ...
                            question = new Question(
                                    ...
                                    false);
                            // Create the answer and include the export rules
                            DateAnswer dateAnswer = new DateAnswer(question, true, startDate, endDate);
                            for (ExportTemplate exportTemplate : exportTemplates) {
                        ...
                        case BOOLEAN: {
                            LOGGER.debug(
                                    "The DataType of ItemDef with OID {} was "
                            ...
                            question = new Question(
                                    ...
                                    questionnaire,
                                    false);
                            question.setMinMaxNumberAnswers(DEFAULT_MIN_NUMBER_ANSWERS, DEFAULT_MAX_NUMBER_ANSWERS);
                            ...
                            result.addValidationMessage(
                                    ...
                                    new String[] {itemDef.getOID(), dataType.toString(), answerLabelYes, answerLabelNo
                                    });
                            ...
                            for (String localeCode : localizedQuestionTexts.keySet()) {
                                ...
                                if (localizedLabelMap.get(localeCode).equals("")) {
                                    LOGGER.debug("The language code " + localeCode + " has no " + "translation for "
                                            + "boolean questions" + ".");
                                }
                            }

                            // Create the answer and include the export rules
                            SelectAnswer answerTrue =
                                    new SelectAnswer(question, true, localizedLabelMap, Boolean.FALSE);
                            for (ExportTemplate exportTemplate : exportTemplates) {
                            ...
                            for (String localeCode : localizedQuestionTexts.keySet()) {
                                ...
                                localizedLabelMap.put(
                                        localeCode,
                                        messageSource.getMessage(
                                                "survey" + ".question.answer.no", null, currentLocale));
                                if (localizedLabelMap.get(localeCode).equals("")) {
                                    LOGGER.debug("The language code " + localeCode + " has no " + "translation for "
                                            + "boolean questions" + ".");
                                }
                        ...
                        default: {
                            LOGGER.debug(
                                    "The DataType of ItemDef with OID {} was "
                                            ...
                                            + "write an error message and go on.",
                                    itemDef.getOID(),
                                    dataType);
                            result.addValidationMessage(
                    ...
                    // stuff
                    LOGGER.debug(
                            "The ItemDef with OID {} refers to a CodeListRef "
                            ...
                            itemDef.getOID());
                    LOGGER.debug(
                            "All information necessary for creation of a "
                    ...
                    if (codeListList == null) {
                        LOGGER.debug(
                                "The ItemDef with OID {} refers to a "
                                        ...
                                        + "Will add an error message and finish",
                                itemDef.getOID(),
                                codeListOID,
                                metaDataVersion.getOID());
                        result.addValidationMessage(
                        ...
                        for (ODMcomplexTypeDefinitionCodeList codeList : codeListList) {
                            ...
                            if (codeList.getOID().equalsIgnoreCase(codeListOID)) {
                                ...
                                result.addValidationMessage(
                                        "import.odm.v132.itemDef" + ".minMaxNumberAnswersDefaultSet", new String[] {
                                            itemDef.getOID(),
                        ...
                        if (odmAnswerList == null) {
                            ...
                            LOGGER.debug(
                                    "A matching CodeList could be found "
                                            + "for the referenced CodeList with OID {} in "
                                            + "MetaDataVersion with OID {} to create a "
                                            + "multiple choice question for ItemDef with OID "
                                            + "{}. Now checking for CodeListItems.",
                                    codeListOID,
                                    metaDataVersion.getOID(),
                                    itemDef.getOID());
                            List<ODMcomplexTypeDefinitionCodeListItem> codeListItemList =
                                    odmAnswerList.getCodeListItem();
                            if (codeListItemList == null || codeListItemList.isEmpty()) {
                                LOGGER.debug(
                                        "No CodeListItems are inside the "
                                                + "CodeList with OID {}. MoPat only "
                                                + "supports CodeListItems. Will write"
                                                + " an error message and finish",
                                        odmAnswerList.getOID());
                                result.addValidationMessage(
                                        "import.odm.v132.codeList" + ".codeListItemListNullEmpty",
                                        new String[] {odmAnswerList.getOID(), metaDataVersion.getOID(), itemDef.getOID()
                                        });
                            } else {
                                LOGGER.debug(
                                        "The CodeList with OID {} "
                                ...
                                Map<Integer, ODMcomplexTypeDefinitionCodeListItem> codeListItemListOrdered =
                                        new HashMap<Integer, ODMcomplexTypeDefinitionCodeListItem>();
                                for (ODMcomplexTypeDefinitionCodeListItem codeListItem : codeListItemList) {
                                    LOGGER.debug(
                                            "Checking the CodeListItem with " + "CodedValue {} whether it has "
                                                    + "an OrderNumber or not",
                                            codeListItem.getCodedValue());
                                    if (codeListItem.getOrderNumber() != null) {
                                        LOGGER.debug(
                                                "The CodeListItem with "
                                        ...
                                        LOGGER.debug(
                                                ...
                                                codeListItem.getCodedValue());
                                        result.addValidationMessage(
                                                "import.odm.v132.codeList" + ".codeListItemNoOrderNumber",
                                                new String[] {
                                                    codeListItem.getCodedValue(),
                                                    metaDataVersion.getOID(),
                                                    itemDef.getOID()
                                ...
                                if (orderNumberForAllCodeListItemsPresent) {
                                    ...
                                    List<Integer> orderNumbers =
                                            new ArrayList<Integer>(codeListItemListOrdered.keySet());
                                    Collections.sort(orderNumbers);
                                    List<ODMcomplexTypeDefinitionCodeListItem> codeListItemsOrdered =
                                            new ArrayList<ODMcomplexTypeDefinitionCodeListItem>();
                                    for (Integer orderNumber : orderNumbers) {
                                        codeListItemsOrdered.add(codeListItemListOrdered.get(orderNumber));
                                    }
                                ...
                                while (codeListItemIterator.hasNext()) {
                                    ...
                                    Map<String, String> localizedAnswerTexts = getTranslations(
                                            codeListItem.getDecode().getTranslatedText(), minAnswerTextLength);
                                    if (localizedAnswerTexts == null) {
                                        LOGGER.debug(
                                                "No proper TranslatedText for"
                                                ...
                                                codeListItem.getCodedValue());
                                        result.addValidationMessage(
                                                "import.odm.v132.codeList" + ".codeListItemNoTranslatedText",
                                                new String[] {
                                                    codeListItem.getCodedValue(),
                                                    metaDataVersion.getOID(),
                                                    itemDef.getOID(),
                                ...
                                for (Map<String, String> localizedAnswerText : localizedAnswerTextList) {
                                    ...
                                    if (codeListItemStringIterator.hasNext()) {
                                        ...
                                        if (codeListItemString.lastIndexOf(" ") == codeListItemString.length() - 1) {
                                            LOGGER.debug(
                                                    ...
                                                    codeListItemString);
                                            result.addValidationMessage(
                                                    "import.odm.v132.codeList" + ".codedValueLastCharacterSpace",
                                        ...
                                        // question
                                        try {
                                            selectAnswer.setValue(Double.parseDouble(codeListItemString));
                                        } catch (NumberFormatException exception) {
                                            // If the value is not of type
                                            // double ignore it
                                            LOGGER.debug(
                                                    "The given CodedValue {} "
                                            ...
                                            ExportRuleAnswer exportRuleAnswerSelect = new ExportRuleAnswer(
                                                    ...
                                                    itemGroupDef
                                                            ...
                                                            + codeListItemString
                                                                    ...
                                                                    .replace("_", "u005F"),
                                                    selectAnswer);
                                            exportRuleAnswerSelect.setExportRuleFormat(new ExportRuleFormat());
        ...
        if (!formalExpressions.isEmpty()) {
            ...
            for (ODMcomplexTypeDefinitionFormalExpression formalExpression : formalExpressions) {
                ...
                if (formalExpression.getContext().equals("XPATH")) {
                    ImportConditionResult condition = new ImportConditionResult(formalExpression.getValue());
    ...
    private static Map<String, String> getBestFittingTranslatedText(
            List<ODMcomplexTypeDefinitionTranslatedText> translatedTextList, int minLength) {
        LOGGER.debug("Enter getBestFittingTranslatedText" + "(List<ODMcomplexTypeDefinitionTranslatedText>)");
        Map<String, String> result = null;

        boolean textInDeDePresent = false;
        for (ODMcomplexTypeDefinitionTranslatedText translatedText : translatedTextList) {
            LOGGER.debug("Checking translatedText at position {},", translatedTextList.indexOf(translatedText));
            String lang = translatedText.getLang();
            if (lang != null) {
                LOGGER.debug(
                        "lang attribute of TranslatedText at position {} is " + "not null",
                        translatedTextList.indexOf(translatedText));
                if (lang.equalsIgnoreCase(TRANSLATED_TEXT_LANG_DE_DE)) {
                    LOGGER.debug(
                            "lang attribute of TranslatedText at "
                                    + "position {} matches (ignoring case) the wanted lang of" + " {}",
                            translatedTextList.indexOf(translatedText),
                            TRANSLATED_TEXT_LANG_DE_DE);
                    String value = translatedText.getValue().replaceAll("(\\r|\\n)", "");
                    if (value != null && value.trim().length() >= minLength) {
                        LOGGER.debug(
                                "Content (i.e. question text) for "
                        ...
                        LOGGER.debug(
                                "Although the TranslatedText at position"
                    ...
                    LOGGER.debug(
                            "TranslatedText at position {} did not have the " + "matching lang attribute of {}",
                            translatedTextList.indexOf(translatedText),
                            TRANSLATED_TEXT_LANG_DE_DE);
                }
            } else {
                LOGGER.debug(
                        "TranslatedText at position {} did not have a "
        ...
        if (!textInDeDePresent) {
            LOGGER.debug(
                    "No TranslatedText with lang {} and non-empty content (i"
                ...
                if (lang != null) {
                    ...
                    if (lang.equalsIgnoreCase(TRANSLATED_TEXT_LANG_DE)) {
                        ...
                        if (value != null && value.trim().length() >= minLength) {
                            ...
                            LOGGER.debug(
                                    "Although the TranslatedText at "
                    ...
                    LOGGER.debug(
                            "TranslatedText at position {} did not have "
            ...
            if (!textInDePresent) {
                LOGGER.debug(
                        "No TranslatedText with lang {} and non-empty content"
                ...
                for (ODMcomplexTypeDefinitionTranslatedText translatedText : translatedTextList) {
                    ...
                    if (lang == null) { // [bt] now the lang attribute is not
                        ...
                        LOGGER.debug(
                                "lang attribute of TranslatedText at position"
                        ...
                        LOGGER.debug(
                                "TranslatedText at position {} did have a "
        ...
        LOGGER.debug("Leaving getBestFittingTranslatedText" + "(List<ODMcomplexTypeDefinitionTranslatedText>)");
        return result;
    }
     ...
     * @return TODO
     */
    private static Double[] getMinMaxFromRangeCheck(
            ODMcomplexTypeDefinitionItemDef itemDef,
            List<ODMcomplexTypeDefinitionRangeCheck> rangeCheckList,
            ImportQuestionResult result) {
        Double[] minAndMax = new Double[2];
        Double min = null;
        Double max = null;
        if (rangeCheckList != null && !rangeCheckList.isEmpty()) {
            LOGGER.debug(
                    "ItemDef of OID {} contains RangeChecks. Will iterate "
                            + "over them to check for RangeChecks about 'GE' and 'LE'.",
                    itemDef.getOID());
            for (ODMcomplexTypeDefinitionRangeCheck rangeCheck : rangeCheckList) {
                ...
                if (comparator == null) {
                    ...
                    switch (comparator) {
                        case GE: {
                            if (min == null) {
                                LOGGER.debug(
                                        "RangeCheck comparator {} detected. " + "Checking for 'CheckValue'",
                                        comparator);
                                List<ODMcomplexTypeDefinitionCheckValue> checkValueList = rangeCheck.getCheckValue();
                                if (checkValueList == null || checkValueList.isEmpty()) {
                                    LOGGER.debug(
                                            "RangeCheck at position {} does " + "not contain any CheckValue "
                                                    + "elements. Will skip it.",
                                            rangeCheckList.indexOf(rangeCheck));
                                    result.addValidationMessage(
                                            "import.odm.v132.itemDef" + ".rangeCheck.checkValueNullEmpty",
                                            new String[] {
                                                itemDef.getOID(), Integer.toString(rangeCheckList.indexOf(rangeCheck))
                                            });
                                } else {
                                    LOGGER.debug(
                                    ...
                                    for (ODMcomplexTypeDefinitionCheckValue checkValue : checkValueList) {
                                        ...
                                        try {
                                            LOGGER.debug(
                                                    "Trying to parse the xml " + "value ('{}') into an " + "int",
                                                    checkValue.getValue());
                                            ...
                                            LOGGER.debug(
                                                    ...
                                                    nfe);
                                            result.addValidationMessage(
                                                    "import.odm.v132.itemDef" + ".rangeCheck.checkValue" + ".noDouble",
                                                    new String[] {
                                                        itemDef.getOID(),
                                                        Integer.toString(rangeCheckList.indexOf(rangeCheck)),
                                                        Integer.toString(checkValueList.indexOf(checkValue))
                                                    });
                                        }
                                    }
                                }
                                ...
                                LOGGER.debug(
                                        "RangeCheck at position {} has the "
                                                + "comparator {}, but min value is "
                                                + "already set. Won't consider it.",
                                        rangeCheckList.indexOf(rangeCheck),
                                        comparator);
                                result.addValidationMessage(
                                        "import.odm.v132.itemDef" + ".rangeCheckComparatorRedundand", new String[] {
                                            itemDef.getOID(),
                                            Integer.toString(rangeCheckList.indexOf(rangeCheck)),
                                            comparator.toString()
                                        });
                        ...
                        default: {
                            LOGGER.debug(
                                    "RangeCheck at position {} contains a " + "Comparator I don't support ({}).",
                                    rangeCheckList.indexOf(rangeCheck),
                                    comparator);
                            result.addValidationMessage(
                                    "import.odm.v132.itemDef" + ".rangeCheckComparatorUnsupported", new String[] {
                                        itemDef.getOID(),
                                        Integer.toString(rangeCheckList.indexOf(rangeCheck)),
                                        comparator.toString()
                                    });
            ...
            if (min != null && max != null && max <= min) {
                ...
                result.addValidationMessage(
                        "import.odm.v132.itemDef.rangeCheckMaxSmallerMin",
