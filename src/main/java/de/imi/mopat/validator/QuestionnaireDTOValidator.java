...
public class QuestionnaireDTOValidator implements Validator {
    ...
    public void validate(final Object o, final Errors errors) {
        ...
        if (!questionnaireDTO.getName().isEmpty()
                && questionnaireDTO.getName().trim().isEmpty()) {
            questionnaireDTO.setName("");
            errors.rejectValue(
                    "name",
                    MoPatValidator.ERRORCODE_ERRORMESSAGE,
                    messageSource.getMessage(
                            "questionnaire.error.nameIsEmpty", new Object[] {}, LocaleContextHolder.getLocale()));
        }
        ...
        if (!questionnaireDTO.getName().matches("^[\\p{L}0-9\\s\\-_.:()\\[\\]!+?]+$")) {
            errors.rejectValue(
                    ...
                    MoPatValidator.ERRORCODE_ERRORMESSAGE,
                    messageSource.getMessage(
                            "questionnaire.error.nameContainsSpecialCharacters",
                            new Object[] {},
                            LocaleContextHolder.getLocale()));
        }

        if (questionnaireDTO.getName().matches("^[\\p{L}0-9\\s\\-_.:()\\[\\]!+?]+$")
                && !questionnaireDao.isQuestionnaireNameUnique(questionnaireDTO.getName(), questionnaireDTO.getId())) {
            errors.rejectValue(
                    "name",
                    MoPatValidator.ERRORCODE_ERRORMESSAGE,
                    messageSource.getMessage(
                            "questionnaire.error.nameInUse", new Object[] {}, LocaleContextHolder.getLocale()));
        }

        // [sw] Check if any added language contains an empty questionText
        for (Map.Entry<String, String> entry :
                questionnaireDTO.getLocalizedDisplayName().entrySet()) {
            if (entry.getValue() == null
                    || entry.getValue().trim().isEmpty()
                    || Pattern.matches("<p>(<p>|</p>|\\s|&nbsp;|<br>)+<\\/p>", entry.getValue())) {
                errors.rejectValue(
                        "localizedDisplayName[" + entry.getKey() + "]",
                        MoPatValidator.ERRORCODE_NOT_NULL,
                        messageSource.getMessage(
                                "questionnaire.displayName" + ".notNull",
                                new Object[] {},
                                LocaleContextHolder.getLocale()));
            }
        }
        ...
        for (Map.Entry<String, String> entry :
                questionnaireDTO.getLocalizedWelcomeText().entrySet()) {
            ...
            for (Map.Entry<String, String> entry :
                ...
                if (entry.getValue().isEmpty()) {
                    ...
                    errors.rejectValue(
                            ...
                            MoPatValidator.ERRORCODE_NOT_NULL,
                            messageSource.getMessage(
                                    "questionnaire.validator" + ".welcomeText" + ".notNull",
                                    new Object[] {},
                                    LocaleContextHolder.getLocale()));
                }
            }
        ...
        for (Map.Entry<String, String> entry :
                questionnaireDTO.getLocalizedFinalText().entrySet()) {
            ...
            for (Map.Entry<String, String> entry :
                ...
                if (entry.getValue().isEmpty()) {
                    ...
                    errors.rejectValue(
                            ...
                            MoPatValidator.ERRORCODE_NOT_NULL,
                            messageSource.getMessage(
                                    "questionnaire.validator" + ".finalText" + ".notNull",
                                    new Object[] {},
                                    LocaleContextHolder.getLocale()));
                }
            }
