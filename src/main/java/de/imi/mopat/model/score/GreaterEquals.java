 ...
 */
@Entity
@DiscriminatorValue("GreaterEquals")
public class GreaterEquals extends BinaryOperatorBoolean {
    ...
    public Boolean evaluate(final Expression expression, final Encounter encounter) {
        if (expression instanceof BinaryExpression) {
            ...
            List<Expression> greaterEqualsExpressions = binaryExpression.getExpressions();
            if (greaterEqualsExpressions.size() != 2) {
                LOGGER.error("The number of expressions for a greater or " + "equals operator must be 2.");
                return null;
            }
            Iterator<Expression> iterator = greaterEqualsExpressions.iterator();
