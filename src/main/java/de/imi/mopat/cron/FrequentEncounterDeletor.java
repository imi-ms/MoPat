...
public class FrequentEncounterDeletor {
    ...
    private static final Logger LOGGER = org.slf4j.LoggerFactory.getLogger(FrequentEncounterDeletor.class);
    ...
    private static final Long ONEHUNDREDEIGHTY_DAYS_IN_MILLISECONDS = 180L * 24L * 60L * 60L * 1000L;
    ...
    public void deleteOldEncounters() {
        ...
        Timestamp nowMinusFinishedEncounterTimeWindow =
                new Timestamp(System.currentTimeMillis() - finishedEncounterTimeWindowInMillis);
        Timestamp nowMinusInclompleteEncounterTimeWindow =
                new Timestamp(System.currentTimeMillis() - incompleteEncounterTimeWindowInMillis);
        try {
            if (deleteFinishedEncounters) {
                oldEncounters.addAll(encounterDao.getFinishedEncounterOlderThan(nowMinusFinishedEncounterTimeWindow));
            }
            if (deleteIncompleteEncounters) {
                oldEncounters.addAll(
                        encounterDao.getIncompleteEncountersOlderThan(nowMinusInclompleteEncounterTimeWindow));
            }
        } catch (Exception e) {
            LOGGER.error(
                    "Something went wrong while checking for old finished "
        ...
        if (finishedEncounterScheduledTimeWindowInMillis == null) {
            LOGGER.info(
                    "Could not find a value for the property {}; will take " + "the default (90 days) instead",
        ...
        Long incompleteEncounterScheduledTimeWindowInMillis =
        ...
        Timestamp nowMinusFinishedEncounterScheduledTimeWindow =
                new Timestamp(System.currentTimeMillis() - finishedEncounterScheduledTimeWindowInMillis);
        Timestamp nowMinusInclompleteEncounterScheduledTimeWindow =
                new Timestamp(System.currentTimeMillis() - incompleteEncounterScheduledTimeWindowInMillis);
        try {
            if (deleteFinishedEncounterScheduleds) {
                oldEncounterScheduleds.addAll(encounterScheduledDao.getFinishedEncounterScheduledOlderThan(
            ...
            LOGGER.error(
                    "Something went wrong while checking for old finished "
                            ...
                            + "encounters, investigate this error ASAP",
                    e);
        }
        ...
        for (Encounter encounter : oldEncounters) {
            ...
            if (encounter.getEncounterScheduled() == null) {
                try {
                    ...
                    LOGGER.error(
                            "Something went wrong while deleting an old "
                                    + "finished Encounter. Since this is "
                                    + "important for not having a database of "
                                    + "old finished encounters, investigate " + "this error ASAP",
                            e);
                }
            }
        }
